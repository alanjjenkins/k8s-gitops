...
initVault() {
  ...
  while [ $VAULT_READY != 0 ]; do
    kubectl --namespace kube-system wait --for condition=Initialized pod/vault-0 > /dev/null 2>&1
    VAULT_READY="$?"
  ...
  VAULT_READY=1
  while [ $VAULT_READY != 0 ]; do
    init_status=$(kubectl --namespace kube-system exec "vault-0" -- vault status -format=json 2>/dev/null | jq -r '.initialized')
  ...
  sealed_status=$(kubectl --namespace kube-system exec "vault-0" -- vault status -format=json 2>/dev/null | jq -r '.sealed')
  init_status=$(kubectl --namespace kube-system exec "vault-0" -- vault status -format=json 2>/dev/null | jq -r '.initialized')

  if [ "$init_status" == "false" ]; then
    echo "initializing vault"
    vault_init=$(kubectl --namespace kube-system exec "vault-0" -- vault operator init -format json -recovery-shares=1 -recovery-threshold=1) || exit 1
    ...
    for replica in "${REPLIACS_LIST[@]:1}"; do
      ...
      kubectl --namespace kube-system exec "vault-${replica}" -- vault operator raft join http://vault-0.vault-internal:8200 || exit 1
    done
  ...
  if [ "$sealed_status" == "true" ]; then
    ...
    for replica in $REPLIACS; do
      ...
      kubectl --namespace kube-system exec "vault-${replica}" -- vault operator unseal "$VAULT_RECOVERY_TOKEN" || exit 1
    done
  fi
}
...
portForwardVault() {
  ...
  kubectl --namespace kube-system port-forward svc/vault 8200:8200 >/dev/null 2>&1 &
...
loginVault() {
  ...
  if [[ "$?" -ne 0 ]]; then
    ...
    echo "1. port-forward the vault service (e.g. 'kubectl --namespace kube-system port-forward svc/vault 8200:8200 &')"
...
EOF
  ...
  export VAULT_SECRETS_OPERATOR_NAMESPACE=$(kubectl --namespace kube-system get sa vault-secrets-operator -o jsonpath="{.metadata.namespace}")
  export VAULT_SECRET_NAME=$(kubectl --namespace kube-system get sa vault-secrets-operator -o jsonpath="{.secrets[*]['name']}")
  export SA_JWT_TOKEN=$(kubectl --namespace kube-system get secret $VAULT_SECRET_NAME -o jsonpath="{.data.token}" | base64 --decode; echo)
  export SA_CA_CRT=$(kubectl --namespace kube-system get secret $VAULT_SECRET_NAME -o jsonpath="{.data['ca\.crt']}" | base64 --decode; echo)
  export K8S_HOST=$(kubectl --namespace kube-system config view --minify -o jsonpath='{.clusters[0].cluster.server}')
